# new_core/adapters/flux_adapter.py
import os
import time
import base64
...
class FluxAdapter(IImageGenerator):
    def __init__(
        self,
        *,
        model: str,
    ...
    async def generate(self, prompt: DiffusionPrompt) -> Tuple[str, Dict]:
        return await asyncio.to_thread(self._generate_sync, prompt.text)

    # ---------------- internal (sync) ----------------

    def _generate_sync(self, prompt_text: str) -> Tuple[str, Dict[str, Any]]:
        ...
        img_path = str(local_path) if local_path else sample
        meta = {
